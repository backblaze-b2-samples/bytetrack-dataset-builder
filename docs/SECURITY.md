...
Security principles and implementation for bytetrack-dataset-builder.
...
- **Client -> B2**: Presigned URLs for clip playback / release download (short expiry)
- **Dataset deletes are prefix-scoped**: a delete only ever targets a single
  `dataset/<id>/` prefix (`repo/dataset_store.py::delete_prefix` refuses an
...
- Content-type allowlist (video only — raw footage)
