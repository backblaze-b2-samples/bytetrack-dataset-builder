...
## Purpose
List, preview, download, and delete **any** file stored in Backblaze B2 — the full-bucket explorer kept from the starter. (This app also adds a separate, *scoped* [Datasets explorer](datasets-explorer.md) for its own `datasets/` prefixes; the Files explorer here browses the whole bucket, including `sources/` and `datasets/`.)
